...
function initProfile() {
  ...
  if (savedProfile) {
    nameInput.value = savedProfile.fullName || '';
    phoneInput.value = savedProfile.phone || '';
    address1Input.value = savedProfile.addressLine1 || '';
    address2Input.value = savedProfile.addressLine2 || '';
    cityInput.value = savedProfile.city || '';
    stateInput.value = savedProfile.state || '';
    pincodeInput.value = savedProfile.pincode || '';
  } else if (loggedInUser) {
    // Fallback to currently logged in user credentials
    nameInput.value = loggedInUser.name || '';
    phoneInput.value = loggedInUser.phone || '';
    address1Input.value = loggedInUser.address || '';
    address2Input.value = '';
    cityInput.value = '';
    stateInput.value = '';
  ...
  profileForm.addEventListener('submit', (e) => {
    ...
    // Form is valid - Save to userProfile localStorage key
    const profileData = {
      fullName,
      phone,
      addressLine1,
    ...
    // Also update current active session details if name changed
    if (loggedInUser) {
      loggedInUser.name = fullName;
      loggedInUser.phone = phone;
      loggedInUser.address = addressLine1;
      loggedInUser.pincode = pincode;
      localStorage.setItem('neo_logged_in_user', JSON.stringify(loggedInUser));
    }
